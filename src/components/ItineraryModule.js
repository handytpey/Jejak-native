...
import {
  View, Text, TextInput, TouchableOpacity, FlatList, Image, StyleSheet, ActivityIndicator, Platform,
} from 'react-native';
...
export default function ItineraryModule({ projectId, userId, memberNames }) {
  ...
  const [draft, setDraft] = useState({ title: '', date: '', time: '', location: '', notes: '', photoUrls: [] });
  ...
  function cancelForm() {
    setDraft({ title: '', date: '', time: '', location: '', notes: '', photoUrls: [] });
    setPhotoUris([]);
    setEditingId(null);
    setShowAdd(false);
  }
            ...
            <View style={[styles.addCard, { marginBottom: 14 }]}>
              ...
              <View style={{ flexDirection: 'row', gap: 12, marginBottom: 10 }}>
                {Platform.OS === 'web' ? (
                  ...
                  <>
                    <TouchableOpacity style={[styles.input, styles.pickerBtn, { flex: 1 }]} onPress={() => setShowDatePicker(true)}>
                      <Text style={draft.date ? styles.pickerBtnText : styles.pickerBtnPlaceholder}>
                        {draft.date ? formatDate(draft.date) : 'Pick a date'}
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.input, styles.pickerBtn, { flex: 1 }]} onPress={() => setShowTimePicker(true)}>
                      <Text style={draft.time ? styles.pickerBtnText : styles.pickerBtnPlaceholder}>
                        {draft.time || 'Pick a time'}
                      </Text>
                    </TouchableOpacity>
                  </>
                )}
